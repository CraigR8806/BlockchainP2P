...
def main():
    ...
    if properties["pki"]["enabled"]:
        pki = PKI(
            properties["pki"]["certificate_path"],
            properties["pki"]["private_key_path"],
            properties["pki"]["certificate_authority_path"],
        )
        connection = Connection(
            properties["server"]["host"], properties["server"]["https_port"]
        )
        bootstrap_connections = [
            Connection(c["host"], c["https_port"])
            for c in properties["client"]["bootstrap_connections"]
        ]
    else:
        pki = NoPKI()
        connection = Connection(
            properties["server"]["host"], properties["server"]["http_port"]
        )
        bootstrap_connections = [
            Connection(c["host"], c["http_port"])
            for c in properties["client"]["bootstrap_connections"]
        ]

    database_connection = Connection(
        properties["database"]["host"], properties["database"]["port"]
    )
    wait_time = 2
