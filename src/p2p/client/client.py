...
class Client:
    ...
    def get_all(
        self, uri: str, params: t.Dict[str, str], logger: Logger = None
    ) -> t.Dict[Peer, Response]:
        """
        ...
        return self.get_some(
            self.__data_service.deep_copy("active_peers"),
            uri,
            params=params,
            logger=logger,
        )
    ...
    def get_some(
        ...
        if asyync:
            asyncio.run(
                self.__async_call(
                    peers, responses, self.__get, uri, params=params, logger=logger
                )
            )
        else:
            for peer in peers:
                responses.update(
                    {
                        peer: self.__get(
                            peer.get_connection(), uri, params=params, logger=logger
                        )
                    }
                )
        return responses
    ...
    def post_all(
        self, uri: str, data: any, logger: Logger = None
    ) -> t.Dict[Peer, Response]:
        """
        ...
        Returns:
            t.Dict[Peer, Response]: A `Dict` of `Peer` to that `Peer`'s `Response`
        """
        return self.post_some(
            self.__data_service.deep_copy("active_peers"), uri, data, logger=logger
        )
    ...
    def post_some(
        ...
        if asyync:
            asyncio.run(
                self.__async_call(
                    peers, responses, self.__post, uri, data, logger=logger
                )
            )
        else:
            for peer in peers:
                responses[peer] = self.__post(
                    peer.get_connection(), uri, data, logger=logger
                )
    ...
    async def __async_call(
        self,
        peers: t.Iterable[Peer],
        responses: t.Dict[Peer, Response],
        func: t.Callable,
        *args,
        **kwargs
    ) -> None:
    ...
    def post_one(
        self, connection: Connection, uri: str, data: any, logger: Logger = None
    ) -> Response:
        """
    ...
    def __get(
        ...
        return self.__retry_if_fail(
            lambda: requests.get(
                ...
                cert=self.__pki.get_ssl_context(),
                params=params,
            ),
            retry_on_fail,
