...
class DataService:
    ...
    class __DataServiceQueueEntry:
        ...
        def __init__(self, key:str, func:Callable, args:Iterable[Any], query_type:'DataService.__DataServiceQueryQueryTypeEnum', condition:Condition = None, callback:Callable = None):
            self.key = key
            self.func = func
            self.args = args
    ...
    def modify(self, key:str, func:DataAccessor, condition:Condition = None, callback:Callable = None, asyync:bool = False) -> None:
        ...
        entry = DataService.__DataServiceQueueEntry(key, func, [self.__data_map.get(key, None)], DataService.__DataServiceQueryQueryTypeEnum.MODIFY, condition, callback)
    ...
    def remove_watch(self, key:str, uuid:UUID) -> None:
        self.__watch_map[key].remove([watch for watch in self.__watch_map[key] if watch.get_uuid() == uuid][0])
    ...
    def __listen(self) -> None:
        ...
        while self.__running:
            ...
            if len(self.__queue):
                ...
                if entry.query_type == DataService.__DataServiceQueryQueryTypeEnum.MODIFY and entry.key in self.__watch_map and len(self.__watch_map[entry.key]):
                    for watch in self.__watch_map[entry.key]:
                        Thread(target=watch.get_function()).start()
                        

                if entry.callback is not None:
                    exec_callable_with_timeout(entry.callback, [], self.__timeout_duration)
    ...
    def upsert(self, key:str, val:Any, condition:Condition = None, asyync:bool = False) -> None:
        """
        ...
        entry = DataService.__DataServiceQueueEntry(key, self.__add_it, [key, val], DataService.__DataServiceQueryQueryTypeEnum.UPSERT, condition, lambda :self.__watch_map.update({key: []}) if self.__watch_map.get(key) is None else None)
